		...
		d3.json("./data/edge.json", function(error, edge){
			...
			function render_sentence(index){
				...
				function newedge()
				{
					...
					return edge_list;//write function for traversal
				}
				/////check this for edge 18 and 19 
				...
				function checktextrange(min_text,max_text,id){
					...
					if(parseInt(number_s) >=parseInt(min_text.wordindex) && parseInt(number_s) <=parseInt(max_text.wordindex)){
						var coor=[];
						coor[0]= getsourcex(id.sourceid).sourcex;			//returns edge why change if possible uses lot of space
				...
				function check_src(map_edge,that_edge){
					if(that_edge.flag==0 || that_edge.flag==2){//normal get dest}
						var dummy = getsourcex(that_edge).srcx;
						map_edge.sourcex=dummy;
						dummy= getsourcey(that_edge).srcy;
						map_edge.sourcey = dummy;
						...
						var dummy = getssourcex(that_edge).desx;
						map_edge.sourcex = dummy;
						dummy= getsourcey(that_edge).desy;
						map_edge.sourcey = dummy;
					}
					return map_edge;
				}
				...
				function check_dest(map_edge,that_edge){
					...
					else if(edg.flag==2){//source only get source
						var dummy =getsourcex(that_edge).srcx;
						map_edge.destx =dummy;
						dummy= getsourcey(that_edge).srcy;
						map_edge.desty = dummy;
					...
					for( var l=0;l < edge_list.length ;l++)
					{
						...
						if(edge_list[l].sourceid.includes("T") && edge_list[l].destinationid.includes("T")){
							...
							if(parseInt(src[1]) >= parseInt(min_text.wordindex) && parseInt(src[1]) <= parseInt(max_text.wordindex)){		//in range check destination edge
								if(edge_list[l].destinationid in edge_map)	{				//both present in range
									edge_map[id]=assign_coors(edge_map[id],edge_list[l],2);
									edge_map[id].flag =0;
									//when edge present in edge map is it with flag=1 or flag=2or flag=0
									var destid = edge_list[l].destinationid;
									var edg = search_edge(edge_list,destid);
									edge_map[id]=check_dest(edge_map[id],edg);
							...
							if(edge_list[l].destinationid in edge_map)	{
								edge_map[id] = check_dest(edge_list[id],edge_list[l].destinationid);
								edge_map[id].flag=1;
								//edge_map[id] = assign_coors(edge_map[id],edge_list[l],1);
							}
							else{		//destination edge not there
						...
						if(parseInt(des[1]) >= parseInt(min_text.wordindex) && parseInt(des[1]) <= parseInt(max_text.wordindex)){		//in range check destination edge
							if(edge_list[l].sourceid in edge_map)	{				//both present in range
								...
								var edg = search_edge(edge_list,sourceid);
								edge_map[id] = check_src(edge_map[id],edge_lis[l]);
								/*if(edg.flag==0 || edg.flag==2){//normal get dest}
								//else if(flag==1){//source only get dest
								var dummy = getsourcex(edg).srcx;
								edge_map[id].sourcex=dummy;
								dummy= getsourcey(edg).srcy;
								edge_map[id].sourcey = dummy;
								}
								else if(edg.flag==1){//dest only get source}
								var dummy = getssourcex(edg).desx;
								edge_map[id].sourcex = dummy;
								dummy= getsourcey(edg).desy;
								edge_map[id].sourcey = dummy;
								}*/
								//edge_map[id].sourcex = getsourcex(edge_list[l]).srcx;
								//edge_map[id].sourcey = getsourcey(edge_list[l]).srcy;
								//var dummy= getdestx(edge_list[l]).desx;
								//edge_map[id].destx = dummy;
								//dummy= getdesty(edge_list[l]).desy;
								//edge_map[id].desty = dummy;
							}
...
//								stack_des.push({parent:edge[l],edge:edge[l].sourceid,flag:1});
								...
								if(coor[0]!=0){
								edge_map[id]= assign_coors(edge_map[id],edge_list,1);
								//edge_map[id] = edge_list[l];
								edge_map[id].flag =0;
							...
							if(edge_list[l].sourceid in edge_map)	{				//both present in range
								...
								var edg_s = search_edge(edge_list,srcid);
								var dummy;
								if(edg_s.flag==0 || edg_s.flag==2){
									dummy = getsourcex(edg_s).srcx;
									edge_map[id].sourcex = dummy;
									dummy= getsourcey(edg_s).srcy;
									edge_map[id].sourcey = dummy;
								}
								else if(edg_s.flag==1){
									dummy = getdestx(edg_s).desx;
									edge_map[id].sourcex = dummy;
									dummy= getdesty(edg_s).desy;
									edge_map[id].sourcey = dummy;
								}
					...
					if(edge_list[l].sourceid in edge_map){
						if(edge_list[l].destinationid in edge_map)	{				//both present in range
							...
							var destid = edge_list[l].destinationid;
							var edg = search_edge(edge_list,destid);
							if(edg.flag==0 || edg.flag==1){//normal get dest}
							//else if(flag==1){//dest only get dest
							dummy=getdestx(edg).desx;
							edge_map[id].destx = dummy;
							dummy= getdesty(edg).desy;
							edge_map[id].desty = dummy;
							}
							else if(edg.flag==2){//source only get source}
							dummy= getssourcex(edg).srcx;
							edge_map[id].destx = dummy;
							dummy= getsourcey(edg).srcy;
							edge_map[id].desty = dummy;
							}
							...
							if(coor[0]!=0){
								edge_map[id]= edge_list[l];
								edge_map[id].flag=0;
								var dummy= getsourcex(edge_list[l]).srcx;
								edge_map[id].sourcex = dummy;
								dummy= getsourcey(edge_list[l]).srcy;
								edge_map[id].sourcey = dummy;
								edge_map[id].destx= coor[0];
								edge_map[id].desty=coor[1];
							}
						...
						if(edge_list[l].destinationid in edge_map)	{				//destination present in range
							...
							var dummy= getdestx(edge_list[l]).desx;
							edge_map[id].destx = dummy;
							dummy= getdesty(edge_list[l]).desy; // this block doubtful
							edge_map[id].desty = dummy;
						}
